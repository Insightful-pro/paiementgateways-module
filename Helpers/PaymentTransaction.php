...
class PaymentTransaction
{
        ...
        public function TransactionConfirmed($transaction_id,$payment_response='')
        {
            ...
            $payment_date = Carbon::now();
                ...
                $transaction->save();
                    ...
                    $meta=[
                        'title' => 'Alimentation via '.$transaction->modePaiement->libelle,
                        'description'=> 'Référence de paiement '.$transaction->reference,
        ...
        public function TransactionRejected($transaction_id,$payment_response='')
        {
           ...
           // PortMonnaieHelper::updateSolde($transaction->user,$transaction);
            ...
            if($transaction->callbackfunction)
            {
                call_user_func_array($transaction->callbackfunction,[$transaction->id,'rejected']);
            }
            ...
            try
            {
                ...
                $transaction =Paiement::find($transaction_id);
                    ...
                    [
                        ...
                        //'price' => $transaction,//ici correction 3
                        'price' => $transaction->montant+($transaction->frais_variable??0)+($transaction->frais_fixe??0),
                        'qty' => 1//$item->quantity
                    ]);
                ...
                $cart= [
                    ...
                    'invoice_id' =>  config('paiementgateways.prefix_transaction').$transaction->id ,
                    ...
                    'total' => $transaction->montant+($transaction->frais_variable??0)+($transaction->frais_fixe??0),
                ];
        ...
        if($ModePaiement_id != 0)
        {
            ...
            return response()->json([
                ...
                'success' => 1
                    ...
                    //$amount=$transaction->montant;//ici correction 3
                    $amount  = ($transaction->montant+($transaction->frais_variable??0)+($transaction->frais_fixe??0))*100;
