...
class PaymentNotifyController extends Controller
{
    ...
    public function paypalnotifyapplication(Request $request)
    {
        ...
        if ($post['invoice']!="")
        {
            ...
            $transaction_id = str_replace(config('paiementgateways.prefix_transaction'),'',$post['invoice']);
    ...
    public function stripePostv3(Request $request)
    {
        ...
        if( $transaction!=null)
        {
            \Stripe\Stripe::setApiKey(config('paiementgateways.stripe.STRIPE_SECRET'));
                ...
                //$amount                 = $transaction->montant*100;//ici correction 3
            ...
            $log['items']=$items;
            return response()->json(['error' => $log], 400);
        }

        return response()->json(['error' => $transaction_id.'Transaction not found.'], 401);
    }
    ...
    public function refuse(Request $request){
       ...
       /* $transaction->status = 4;
        ...
        if (auth()->check())
        {
            //return redirect($redirect_url)//ici correction 2
            return redirect($myrequest->redirect_url)
